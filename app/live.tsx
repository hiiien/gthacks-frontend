...
import { StyleSheet, View, Pressable, FlatList, ActivityIndicator, RefreshControl } from 'react-native';
...
// Mock data for the streams. In a real app, this would come from an API.
const STREAMS_DATA = [
...
const StreamCard = ({ item }: { item: Stream }) => {
  ...
  const handlePress = () => {
    // Navigate to a dynamic route for the specific stream
    router.push(`/stream/${item.id}`);
  };
      ...
      {({ pressed }) => (
        ...
        >
          <View className="w-full aspect-video relative">
            ...
            <View className="absolute top-3 left-3 bg-indigo-600 px-2 py-1 rounded-md">
      ...
      <>
        <View className="flex-row gap-3">
          <Pressable 
            ...
            className={`flex-row items-center gap-2 px-4 py-2 rounded-full ${activeFilter === 'hot' ? 'bg-blue-500' : 'bg-neutral-200 dark:bg-neutral-800'}`}>
            <IconSymbol name="flame.fill" size={14} color={activeFilter === 'hot' ? '#FFFFFF' : '#808080'} />
